function [] = Main_Traj_Week(varargin)
% input parameters
...
% changing partners
test_distr = readmatrix(fullfile('Configuration', ['test_distr_pars_',num2str(par_counter),'.csv']));
...
Pop_hiv.Data(hiv_index.ever_tested,:) = 0;
        ...
        if numel(ind_new_bracket)>0 % update the data for individuals who moved into the new bracket
            ...
            if prep_fl & sum(old_prop>Population.Data(pop_index.casual_prop,ind_new_bracket))
                ...
                if sum(cond) > 0
                    ...
                    [Pop_hiv, inds_changed_PreP, PrEP_inds] = update_PrEP(Pop_hiv, Population, alive_ind, infect_alive, pop_index, hiv_index, HIV_st, ind_change, old_prop_change, cfg_PrEP, test_edge, PrEP_inds, test_distr, rec_casual_parts, test_rates);
...
% %        death module 
   ...
   % tic
    ...
    [Population,Pop_hiv,Id_counter,alive_ind,ind_new_bracket, newly_infect3, new_diagn, new_suppr, infect_alive] = Birth_Immigr(Population, Pop_hiv, pop_index, hiv_index,lambda_w,tcounter,Id_counter,alive_ind, cfg, lambda_diagn_w, infect_alive, HIV_st, AgeBirthProp);
...
%           tic 
            ...
            if numel(alive_ind.Data)>numel(infect_alive.Data) % there are individuals who are not infected
                 [Pop_hiv, PrEP_inds] = PrEPUptake_prop(Population, Pop_hiv, hiv_index, HIV_st, alive_ind, infect_alive, cfg, pop_index, PrEP_inds, cfg_PrEP);
            end
