function [Pop_hiv, inds_changed_PreP, PrEP_inds] = update_PrEP_thresh(Pop_hiv, Population, alive_ind, infect_alive, pop_index, hiv_index, HIV_st, ind_list, old_prop, cfg_prep, test_edge, PrEP_inds, test_distr, rec_casual_parts, test_rates, cfg, current_year)
...
indices = ind_list(new_prop < cfg.prep_thresh);
current_props = new_prop(new_prop < cfg.prep_thresh);
...
if numel(indices) > 0
    ...
    for ind = indices
        ...
        % max_prop = datasample(all_propensities(all_propensities>=cfg.prep_thresh), 1); % max(all_propensities(all_propensities>current_props(counter)));
        
        if numel(ind_non_infect(all_propensities>=cfg.prep_thresh))>0
            ind_repl = datasample(ind_non_infect(all_propensities>=cfg.prep_thresh), 1);
            ind_ind_repl = find(ind_non_infect == ind_repl);
        else
            ind_repl = [];
            ind_ind_repl =[];
        ...
        if numel(ind_repl)>0
         ...
         %   ind_repl = ind_non_infect(find(all_propensities == max_prop, 1));
            ...
            ind_non_infect(:, ind_ind_repl) = [];
            all_propensities(:, ind_ind_repl) = [];
