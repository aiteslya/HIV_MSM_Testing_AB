            ...
            if n_infect>0
                ...
                if tcounter>2*year
                    [scenarioName, pct1, pct2, pct3] = parseSimType(sim_type);
                        ...
                        if isequal(scenarioName,'immigr') | isequal(scenarioName,'immigr_late') | isequal(scenarioName,'immigr_parts')

                            perc_diagn = pct1/100; % convert percentages to fractions

                            r = rand(1,numel(n_infect));

                            ind_test_diagn = ind_infect(r<perc_diagn);

                            % check that indices generated are unique
                            assert(numel(ind_test_diagn)==numel(unique(ind_test_diagn)));

                            ind_immigr_infect_test = ind_test_diagn;

                            Pop_hiv.Data(hiv_index.status, ind_test_diagn) = Pop_hiv.Data(hiv_index.status, ind_test_diagn) + 7;

                            Pop_hiv.Data(hiv_index.date_diagn, ind_test_diagn) = tcounter;
                            Pop_hiv.Data(hiv_index.ever_tested, ind_test_diagn) = tcounter;
                            
                            if numel(ind_test_diagn)>0
                                for ind_diagnosed = ind_test_diagn
                                    id = Population.Data(pop_index.id,ind_diagnosed);
                                    HIV_st_id = Pop_hiv.Data(hiv_index.status, ind_diagnosed)-7;
                                    num_parts_id = numel(retrievePartners(rec_casual_parts, id));
                                    record = {id ind_test_diagn tcounter HIV_st_id, Population.Data(pop_index.age,ind_test_diagn) ...
                                                    Population.Data(pop_index.casual_prop,ind_test_diagn) ...
                                                    tcounter - Pop_hiv.Data(hiv_index.date_inf,ind_test_diagn) num_parts_id }; 
