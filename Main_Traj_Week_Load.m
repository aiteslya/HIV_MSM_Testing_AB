function [] = Main_Traj_Week_Load(varargin)
% input parameters
...
elseif nargin ~= 0
    error([mfilename, ': Function expects either 0 or 3 input arguments.']);
end
...
%% Load previously saved state of the system

% Define the folder strings
folder_str_state = 'State';
folder_str_state_pers = ['State_par_', num2str(par_counter), '_batch_', num2str(batch_n)];
...
% array keeping track of indices of live individuals who have HIV
% dynamically changing list
% Check if the file exists before loading
...
% adjust dates in population
% adjust the dates
% duration of calibration rounds in days
dur_calibr_weeks = floor(10.5*year_week);

Population.Data(pop_index.birth, alive_ind.Data) = - Population.Data(pop_index.age, alive_ind.Data);
Population.Data(pop_index.sp1_stdate, alive_ind.Data) = Population.Data(pop_index.sp1_stdate, alive_ind.Data) - dur_calibr_weeks;
...
% adjust date of the last test
ever_tested_indices = alive_ind.Data(Pop_hiv.Data(hiv_index.ever_tested, alive_ind.Data)>0);

Pop_hiv.Data(hiv_index.ever_tested, ever_tested_indices) = Pop_hiv.Data(hiv_index.ever_tested, ever_tested_indices) - dur_calibr_weeks;

% adjust the date of infection
Pop_hiv.Data(hiv_index.date_inf, infect_alive.Data) = Pop_hiv.Data(hiv_index.date_inf, infect_alive.Data) - dur_calibr_weeks+1;
...
Pop_hiv.Data(hiv_index.date_diagn, inds_diagn) = Pop_hiv.Data(hiv_index.date_diagn, inds_diagn) - dur_calibr_weeks+1;

% adjust the date of art initiation
inds_art = inds_diagn(Pop_hiv.Data(hiv_index.status, inds_diagn)>=HIV_st.A1T);
Pop_hiv.Data(hiv_index.date_art, inds_art) = Pop_hiv.Data(hiv_index.date_art, inds_art) - dur_calibr_weeks+1;

% adjust the date of suppression
inds_suppr = inds_art(Pop_hiv.Data(hiv_index.status, inds_art) >= HIV_st.A1S);
Pop_hiv.Data(hiv_index.date_sup, inds_suppr) = Pop_hiv.Data(hiv_index.date_sup, inds_suppr) - dur_calibr_weeks+1;

% adjust the date of moving to the next phase
inds_HIV_trans = setdiff(infect_alive.Data, inds_suppr);
Pop_hiv.Data(hiv_index.date_new_stage, inds_HIV_trans) = Pop_hiv.Data(hiv_index.date_new_stage, inds_HIV_trans) - dur_calibr_weeks;

% assert that for non-suppressed people, no one has the next date sooner than t = 2

assert(~any(Pop_hiv.Data(hiv_index.date_new_stage, inds_HIV_trans)<1));
...
Rels_steady.Data(:,3:4) = Rels_steady.Data(:,3:4) - dur_calibr_weeks + 1;
...
Rels_steady_hist.Data(:,3:4) = Rels_steady_hist.Data(:,3:4) - dur_calibr_weeks + 1;
...
for k = 1:length(keysList)
    ...
    for p = all_casual_parts(currentKey)
        p.start_date = p.start_date - dur_calibr_weeks + 1;
        p.end_date = p.end_date - dur_calibr_weeks + 1;
...
% load Population
% File name for loading the object
filename = fullfile(fullFolderPath, 'PrEP_inds.mat');

% Check if the file exists before loading
if exist(filename, 'file')
    loadedData = load(filename, 'PrEP_inds');
    PrEP_inds = loadedData.PrEP_inds;
    disp('PrEP_inds data loaded successfully.');
else
    error('File does not exist. Check the path and filename.');
end
...
ConfFolderStr='Configuration_forward';
...
OutFolderStr=[out_folder_str,'_pars_',num2str(par_counter),'_batch_',num2str(batch_n),'_',sim_type  ];
...
% changing partners
test_distr = readmatrix(fullfile('Configuration_forward', ['test_distr_pars_',num2str(par_counter),'.csv']));

c_10 = 1/cfg.AgeBr;% ageing rate assuming 10-year age bracket, years^{-1}

T_burn_d = 0; %year_week*cfg.T_burn; % convert burn-in time of demographics and sexual networks from days to years
...
prop(1:16) = AgeBirthProp(1:16);
prop(16:num_year) = delta_p;
c = ones(1, num_year);
mu = nan(1, num_year);
...
%[lambda, N_solutions] = solve_lambda(cfg.N, mu, c, prop);
[lambda, ~] = solve_lambda(cfg.N, mu, c, prop);
...
test_group.AHI = 1;
test_group.parts_11 = 2;
test_group.parts_25 = 3;

Test_rates = zeros(N_steps,numel(fieldnames(test_group)));
...
% Get alive indices and corresponding IDs
alive_idx = alive_ind.Data;  % indices into columns of Population.Data
num_alive = numel(alive_idx);

% Get corresponding IDs from the first row of Population.Data
alive_ids = Population.Data(1, alive_idx);

% Preallocate count vector
partner_counts = zeros(2, num_alive);  % initialize to zero
partner_counts(1, :) =  alive_ind.Data;
% Loop over alive individuals
for i = 1:num_alive
    id = alive_ids(i);
    if isKey(rec_casual_parts, id)
        partner_structs = rec_casual_parts(id);  % 1×N struct array
        partner_counts(2, i) = numel(partner_structs);
    end
end
...
for tcounter=2:1:(year_week*cfg.T)+1 % convert time from years to weeks
    ...
    if mod(tcounter,year_week)==0
        ...
        if numel(ind_new_bracket)>0 % update the data for individuals who moved into the new bracket
            ...
            if sum(old_prop>Population.Data(pop_index.casual_prop,ind_new_bracket))
...
% %        death module 
    ...
    current_year = 2024 + floor(tcounter/year_week);
    ...
    if numel(reset_casual_prop)>0
        ...
        if sum(old_prop>Population.Data(pop_index.casual_prop,reset_casual_prop))
   ...
   % tic
    ...
    % end
    [Population,Pop_hiv,Id_counter,alive_ind,ind_new_bracket, newly_infect3, new_diagn, new_suppr, infect_alive, ind_immigr_infect_test] = Birth_Immigr(Population, Pop_hiv, pop_index, hiv_index,lambda_w,tcounter,Id_counter,alive_ind, cfg, lambda_diagn_w, infect_alive, HIV_st, AgeBirthProp, sim_type, rec_casual_parts, testingLog);
    ...
    if num_new_born > 0 
        ...
        if numel(ind_immigr_infect_test)>0
            total_diagn = [total_diagn; tcounter numel(ind_immigr_infect_test)];
            total_tested = [total_tested; tcounter numel(ind_immigr_infect_test)];
        end
    ...
    if num_pairs_new>num_pairs_old % new pairs were created
        ...
        if ~isempty(ids_steady)
            ...
            if sum(old_prop>Population.Data(pop_index.casual_prop, inds_steady))
...
%      toc  
    ...
    if mod(tcounter,4)==0 & tcounter>half_year
        ...
        % collecting the number of partbers
    
        % disp('Partners accounting')
        % tic
       % Get alive indices and corresponding IDs
        alive_idx = alive_ind.Data;  % indices into columns of Population.Data
        num_alive = numel(alive_idx);
...
% 
    ...
    if burn_fl % burn in is over, run the HIV dynamics
        ...
        if num_died_HIV>0
            ...
            % May 20 2025
            ind_immigr_infect_test = setdiff(ind_immigr_infect_test, ind_dead);
        end
...
%      
    ...
    %toc
        ...
        if numel(infect_undiagn_old)>0 % if number of infected and undiagnosed prior to this stage is above zero
            %
            [Pop_hiv, newly_diagn,prop_diagn_AHI,new_total_diagn_ind,new_AHI_diagn_ind, PrEP_inds, newly_tested, infect_diagn_time, testingLog] = HIV_test_load(Pop_hiv,hiv_index,HIV_st,infect_undiagn_old,prop_diagn_AHI,tcounter,new_total_diagn_ind,new_AHI_diagn_ind,PrEP_inds, alive_ind, infect_diagn_time, testingLog, Population, rec_casual_parts, pop_index, sim_type, partner_counts);
            
            if numel(newly_diagn)>0
                total_diagn = [total_diagn; tcounter numel(newly_diagn)];
            end

            if numel(newly_tested)>0
                total_tested = [total_tested; tcounter numel(newly_tested)];
            end
...
% % % % % % 
        ...
        % are newly diagnosed
        total_infect_undiagn_ind.Data=[setdiff(total_infect_undiagn_ind.Data,newly_diagn), newly_infect1, newly_infect2, setdiff(newly_infect3, ind_immigr_infect_test)];
        ...
        % treatment
        total_diagn_ind = [total_diagn_ind, new_diagn, ind_immigr_infect_test];
%        toc
% % 
% %             % get suppressed
% %             
...
%       points
    ...
    if mod(tcounter,write_step)==0
        ...
        test_rate_AHI = mean(Pop_hiv.Data(hiv_index.test_rate, infect_alive.Data(Pop_hiv.Data(hiv_index.status,infect_alive.Data)<HIV_st.suscPrep)));
        Test_rates(twrite_counter,test_group.AHI) = test_rate_AHI;

        ind_rel = partner_counts(1,(ismember(partner_counts(1,:),alive_ind.Data) & partner_counts(2,:)>=11));

        test_rate_parts_11 = mean(Pop_hiv.Data(hiv_index.test_rate, ind_rel));
        Test_rates(twrite_counter,test_group.parts_11) = test_rate_parts_11;

        ind_rel = partner_counts(1,(ismember(partner_counts(1,:),alive_ind.Data) & partner_counts(2,:)>=25));

        test_rate_parts_25 = mean(Pop_hiv.Data(hiv_index.test_rate, ind_rel));

        Test_rates(twrite_counter,test_group.parts_25) = test_rate_parts_25;
...
Test_rates_TS(1, :)=interp1(twrite,(Test_rates(:,1))',tdistr);
Test_rates_TS(2, :)=interp1(twrite,(Test_rates(:,2))',tdistr);
Test_rates_TS(3, :)=interp1(twrite,(Test_rates(:,3))',tdistr);
...
writematrix(Test_rates_TS,[OutFolderStr,'/Test_rates_TS.csv']);
...
disp(['Simualtion run for parameter set number ', num2str(par_counter),' and trajectory number ', num2str(batch_n)]);
assert(sum(Pop_hiv.Data(hiv_index.status,total_infect_undiagn_ind.Data)>HIV_st.LA)==0);
